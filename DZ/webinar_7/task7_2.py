...
class PersonInfo:
    ...
    def __init__(self, name_surname: str, age: int, *subdivision):
        """
    ...
    def short_name(self):
        """
        Функция получает строку "Имя Фамилия", возвращает строку в формате "Фамилия И."
        :return: видоизмененную строку
        """
    ...
    def new_salary(self):
        """
        Возвращает проиндексированную зарплату по формуле:
        1337*Возраст*суммарное кол-во вхождений трех наиболее часто встречающихся букв из списка подразделений
        """
        subdivision_str = ''.join(self.subdivision)
        my_counter = Counter(subdivision_str)  # создание счетчика
        element_3 = my_counter.most_common(3)  # самые встречаемые 3 элемента
        sum_element_3 = sum(a[1] for a in element_3)
        salary = 1337 * self.age * sum_element_3
        return salary
