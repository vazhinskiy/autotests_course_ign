...
def repeats(our_str):
    """
    Принимает на вход строку, смотрит, сколько раз каждый символ уже встречался
    Возвращает строку, добавляет количество к символам с помощью постфикса формата _n
    :param our_str: Данная строка
    :return: Новая строка
    """
    ...
    new_dict = {}
    for a in our_str:
        if not new_dict.get(a):
            new_dict[a] = 1
        else:
            new_dict[a] = new_dict.get(a) + 1
        new_str += a + f'_{new_dict.get(a)}'
    return new_str
